...
import { Video, Category } from "../services/YoutubeClient";
...
export default function Home() {
	...
	const getVideoCategories = async () => {
		console.log("called getVideoCategories");
		try {
			const categories = await youtubeClient.getVideoCategories();
			setVideoCategories([]);
	...
	return (
		<>
			...
			<div className='flex 2xl:flex-row grow justify-center 2xl:flex-wrap w-full max-w-screen-7xl '>
				<Disclosure
					as='div'
					className='py-6 hidden xl:flex flex-shrink-0 xl:flex-col h-full top-menu left-0 overflow-auto '
				>
					{videoCategories.map((category) => (
						<Disclosure.Button
							className={`${
							...
							onClick={() => {
								if (category.current) {
									...
									// Set the clicked category to current: true
				...
				{/* List of Videos */}
				<div className='flex grow justify-center w-full max-w-7xl py-6 sm:px-6 2xl:px-8 '>
					{loading ? (
						<>
							<p>Loading...</p>
						</>
						...
						<>
							<div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-5 gap-4 p-4 '>
